...
from mdnow.cli import _slug
from mdnow.extractor import Extracted
from mdnow.fetcher import FetchResult


def test_slug_fallbacks():
    assert _slug("My Post: A Guide", "https://x.com/p") == "my-post-a-guide"
    assert _slug("Café Crème", "https://x.com/c") == "cafe-creme"
    assert _slug("日本語", "https://x.com/guide/intro") == "guide-intro"   # non-ascii → url path
    assert _slug("!!!", "https://x.com/docs/p7") == "docs-p7"             # symbols → url path
    assert _slug("", "https://example.com/") == "example-com"             # empty → host
...
def test_acquire_thin_static_kept_when_render_fails(monkeypatch):
    monkeypatch.setattr(cli, "StaticFetcher", _static())
